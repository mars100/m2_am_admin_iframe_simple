...
var App = function () {
    ...
    var handleUniform = function () {
        ...
        if (test.length > 0) { // .size() 废弃
            test.each(function () {
                if ($(this).parents(".checker").length === 0) { // .size() 废弃
                    $(this).show();
                    $(this).uniform();
                }
            });
    ...
    return {
        ...
        alert: function (options) {
            ...
            if (!options.container) {
                ...
                } else if (($('body').hasClass("page-container-bg-solid") 
...
var addTabs = function (options) {
    console.log("addTabs: " + JSON.stringify(options));
    ...
    if (findTabPanel(pageId) === null) {
        ...
        if (options.content) {
            ...
            //iframe 加载完成事件
            //$iframe.load(function () { // .load方式已废弃
        ...
        function init($menu_ul, data, level) {
            $.each(data, function (i, item) {
                ...
                if (item.parent_id == 0) {
                    ...
                    // $title.addClass('menu-text').text((i + 1) + "：" + item.text); // 含有菜单自增计数ID
                    $title.addClass('menu-text').text(item.text); // 不显示菜单自增计数ID
                }
                ...
                if (item.children && item.children.length > 0) {
                    ...
                    // 菜单
                    $a.attr("menu-nav-type", "2"); // 菜单
                    if (item.targetType != null && item.targetType === "blank") //代表打开新页面
                    {
                        var href = "window.open('" + item.url + "');";
